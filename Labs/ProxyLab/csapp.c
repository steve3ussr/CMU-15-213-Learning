...
ssize_t rio_writend(rio_t *rp, void *usrbuf, size_t n)
{
    size_t nleft = n;
    ssize_t nwritten;
    char *bufp = usrbuf;
    ...
    while (nleft > 0) {
	if ((nwritten = write(rp->rio_fd, bufp, nleft)) <= 0) {
	    if (errno == EINTR)  /* Interrupted by sig handler return */
		nwritten = 0;    /* and call write() again */
	    else
		return -1;       /* errno set by write() */
...
void Rio_writend(rio_t *rp, void *usrbuf, size_t n) 
{
    if (rio_writend(rp, usrbuf, n) != n)
	unix_error("Rio_writen error");
}
