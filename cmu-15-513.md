...
exception handle 和 procedure 类似，区别是：
...
所有的IO设备都被认为是一种文件 (是一个字节序列); 对于文件有通用的接口:
...
现实世界的大部分机器都是小端序的, 例如192.168.0.1 对应着 `0xC0A80001`, 实际储存的时候用的是`uint32_t`, 在内存里储存的顺序是`01 00 A8 C0`; 而网络是大端序的, 意味着必须要转换他们. 
...
当创建socket的时候, 默认是一个client (active socket), 但listen会将其转化为server (listen socket)
...
`getaddrinfo`中使用的`hints`参数是一个`addrinfo`结构体，用于筛选合适的结果，例如`ai_family=AF_INET/AF_INET6`可以筛选IPv4/v6
...
nleft是剩余需要读取的字节, 函数一直循环读取, 直到nkeft为0或者报错, 此时返回一个非负整数, 表明这次读取了多少个字节. 

循环中分成以下三种情况: 

1. 如果read返回值为0: 正好读取完, 遇到了EOF, 此时可以退出循环
2. 返回值大于0: 读取了一部分, 减少 nleft, 修改 bufp为正确的位置, 然后再次调用read
3. 返回值小于0: 这表明read出现了某种错误, errno中为错误类型. 大部分错误都是致命的, 需要让rio返回-1, 除了EINTR; 如果返回EINTR说明未读取任何值, 需要重新进行

> [socket编程中的EINTR是什么? - 程序员x的文章 - 知乎](https://zhuanlan.zhihu.com/p/665325402)
>
> **慢系统调用**(slow system call)指不会立即返回的系统调用, 可能**永远阻塞**而无法返回。 例如支持网络的调用, 包括read/write, connect, accept等, 都属于这一类。读时, 数据不存在, 需要等待缓冲区有数据输入; 写时, 缓冲区满, 需要等待缓冲区有空闲位置
>
> 如果进程在一个**慢系统调用**(slow system call)中阻塞时，当捕获到某个信号且相应信号处理函数返回时，这个系统调用被中断，调用返回错误，设置errno为EINTR（相应的错误描述为"Interrupted system call"）
>
> 因此EINTR错误的产生是**慢系统调用**和**信号处理函数**组合使用会产生的问题。对于这样的情况，我们就需要人为的对read进行"重启"， 即重新的进行read

#### 3 rio_writen

和`rio_readn`差不多, 不过写操作是不会又不足值的, 只会返回-1或者n (顺利写入的字节数)
...
rio_read 函数是 Linux read 函数的带缓冲的版本. 这是个static函数, 意味着他只会被`csapp.c`中的`rio_`系列函数使用, 而不能被外部其他源文件调用
...
如果rio对象的待读取计数器小于等于0 (rio内部buffer有值, 但是没有被应用程序读取) , 那么这个函数首先会循环地调用read, 并令`rio_object.rio_cnt = read`. 循环里的情况包括:

1. read返回负数: 如果对应的errno是EINTR就循环read, 否则函数返回-1
2. 返回0: 函数也直接返回0, 没东西可读
3. 返回正数: 读取了一部分, 将rio_bufptr指向buf开头 (==为什么要这样==); 不会进入下一次循环

这个循环能持续运行的条件是不断收到中断信号, 而循环的结果将是:

1. 返回-1
2. 返回0
3. internal buf中有一些未被读取的字节, 并且cnt和ptr都被设置妥当

如果没有返回, 函数将从internal buf中copy一些字节到user buf, copy的大小是`min(n, rio_cnt)`, 用户要求字节和的rio获取字节的最小值; 修改rio buffer和rio cnt的值

---

这个函数可能的用法:

1. 用户读取1个字节, 实际读取10了10个字节到缓冲区: 由于缓冲区非空, 并不会再次read, 只会memcpy并且修改指针; 
2. 用户读取1字节, 但是缓冲区为空: 调用read并且memcpy

#### 6 rio_readinitb

初始化一个rio对象, 将其与一个fd联系起来, 重置cnt和bufptr

#### 7 rio_readlineb

从一个内部读缓冲区复制一个文本行，当缓冲区变空时，会自动地调用read 重新填满缓冲区
...
这个函数的for循环最多循环maxlen-1次, 因为usrbuf的最后一个是NULL. 每次进入循环, 都会尝试读取一个字节, 接下来可能会:

1. 读取到了1个字节:
   1. 写入usrbuf, 并且移动bufp
   2. 如果这个字节不是换行符, 就n++并且继续循环
   3. 如果这个字节是换行符, 就n++并且退出循环
2. 如果没有读取到字节:
   1. 前面已经读取了一些字节: 退出循环 
   2. 什么也没读取到: return 0
3. 报错: return -1

循环结束的时候, n为实际写入的字节数量+1, 所以最终返回的结果要-1; bufp为usrbuf中下一个待填充的位置, 所以要赋值一个0
...
包装了一层`int open_clientfd(char *hostname, char *port)`, **用于建立到服务器的连接**

这个函数首先需要通过`getaddrinfo`获取可用的socket，以`hostname, port`作为前两个输入，hints.socktype为TCP, hints.ai_flags包括了`AI_NUMERICSERV` (告诉函数我输入的是端口号)和`AI_ADDRCONFIG` (如果主机配置了IPv4就返回v4，如果配置了v6就返回v6，避免无法使用的IP地址)

在获取到结果后，就进入循环, 对结果列表中的每个addrinfo都尝试:

1. 建立socket，使用了socket函数，前几个参数分别为结果中的`ai_family, ai_socktype, ai_protocol`; 如果建立失败就continue
2. 尝试connect，如果成功就退出循环, 如果失败就进行第三步
3. 尝试关闭socket, 关闭成功就继续循环, 否则就报错

函数返回的结果包括:

1. 返回已经connect的socket
2. -1, 表示关闭一个失败的连接时报错, 或者所有连接都失败
3. -2, 表示getaddrinfo时报错

### Open_listenfd

包装了一层`int open_listenfd(char *port) `, **用于建立监听**

这个函数首先使用`getaddrinfo`在本机的指定端口上打开一个socket，使用的flag除了数字端口、适配的IP栈，还加上了`AI_PASSIVE`表明这是个被动的服务端。

在获取到结果后，尝试bind。和`open_clientfd`一样，循环要么报错，要么建立了socket。

随后使用了`setsockopt`函数， 它的作用是配置服务器，使得服务器能够被终止、重启和立即开始接收连接请求。一个重启的服务器默认将在大约30 秒内拒绝客户端的连接请求，这严重地阻碍了调试。

然后是bind和listen，最终返回一个listen_fd
...
1. 输入校验
2. Open_clientfd
3. 初始化rio对象， 将其与clientfd关联起来
4. 建立buffer，然后循环调用Fgets从stdin读取内容到buffer； 如果读取到EOF (Ctrl+D) 就退出循环
   1. rio从buffer写入clientfd
   2. 从clientfd读取一行到buffer
   3. 把buffer输出到stdout
5. 关闭clientfd， 退出
...
1. 参数校验
2. 调用`Open_clientfd`开始监听
3. 循环，
   1. 循环中阻塞在Accept；Accept函数需要listenfd，还需要一个SA结构体的指针+对应的大小，返回的时候除了返回connfd，还会**填充**clientaddr和clientlen，分别填入socket地址和socket地址大小；
   2. 调用getnameinfo，从socket (clientaddr) 中获取client 主机名和端口
   3. echo
   4. 关闭connfd
4. 退出

而echo中先建立一个rio对象和connfd联系起来，然后循环rio_readlineb和rio——writen

## Top-Down: Tiny Server

### main

1. 参数校验
2. listen
3. while loop: accept, doit, close

### doit

1. 读并且解析 request line; 如果读不到就返回, 或者解析后发现不支持就报错
2. 读取 request header lines
3. 解析 URI, 确认是动态还是静态请求, 从URI中分离 file 和 args
4. 检查 file 是否存在, 不存在就404
5. 根据动态/静态请求, 确认 file 是否为普通文件, 以及是否有 R/X 权限
6. 响应动态/静态请求

### strcasecmp

对比字符串, 但是忽略大小写; 如果相同返回0, s1>s2返回正数, 否则返回负数
...
synopsis: `int strstr(char *haystack, char *needle)`

return pointer (to the begin) or NULL

### parse_uri

1. 静态返回1, 动态返回0; 修改filename和cgiargs
2. 如果 uri 中不包含 cgi-bin, 说明这是静态内容
   1. cgiargs=“”
   2. filename=“.”+uri, 拼接为完整路径
   3. 如果filename最后一位是`/`, 自动补全`home.html`
3. 动态内容, 并且URI中包含了`?`
   1. cgiargs=?后面的
   2. 将?修改为NULL
   3. filename为“.”+uri
...
Get file attributes for FILE and put them in BUF, BUF is `struct stat` type
...
- 用`sprintf`向`user string print foramtted string`, 然后通过RIO发送出去
- 判断文件类型, 并且写到`content-type`
- 用Mmap减少了一次CPU拷贝. mmap 函数将被请求文件映射到一个虚拟内存空间。回想我们在第9 . 8节中对 mmap 的讨论， 调用 mmap 将文件 srcfd 的前filesize 个字节映射到一个从地址 srcp 开始的私有只读虚拟内存区域。一旦将文件映射到内存，就不再需要它的描述符了，所以我们关闭这个文件（第20行）。执行这项任务失败将导致潜在的致命的内存泄漏。

### serve_dynamic

- 用`sprintf`向`user string print foramtted string`, 然后通过RIO发送出去
- 如果是父进程, 就wait; 如果是子进程就执行业务
- 设置环境变量`QAUERY_STRING`为cgiargs; 在执行进程里, 通过getenv函数可以获取内容
- Dup2将 stdout 重定向到 connfd; 在执行进程里, 函数只需要printf就可以将结果最终输出到connfd
- execve filename
...
> [探索mmap与零拷贝：开启高效内存共享的奇妙之旅 - 玩转Linux内核的文章 - 知乎](https://zhuanlan.zhihu.com/p/12965198260)
...
![](https://picx.zhimg.com/v2-ce1c0588c623ee217f0bc6d9c15ee679_r.jpg)

很坏的地方在于内核态与用户态之间的拷贝是冗余的, 而且两次拷贝都需要CPU全程参与

### DMA

简单理解就是，在进行 I/O 设备和内存的数据传输的时候，数据搬运的工作全部交给 DMA 控制器，而 CPU 不再参与任何与数据搬运相关的事情，这样 CPU 就可以去处理别的事务。

减少了一次CPU拷贝, DMA负责拷贝数据到 kernel buf

![](https://pic4.zhimg.com/v2-bb260b9284dcc642129f33792d3496fb_1440w.jpg)

### DMA: Still Bad Perf, and Why?

如果服务端要提供文件传输的功能，我们能想到的最简单的方式是：将磁盘上的文件读取出来，然后通过网络协议发送给客户端。

如果使用I/O, 需要调用read读取文件, 再通过write发送

看起来很简单, 但是一共有4次用户态/内核态切换, 2次DMA复制, 2次CPU复制

![](https://pic3.zhimg.com/v2-6580130703001266d8d439fa8dcb5d30_r.jpg)

在一些事务中, 我们并不会在用户空间修改数据, 所以本来不需要搬运到用户空间的; 而且用户空间没有权限操作磁盘或网卡, 所以还需要上下文切换

### Mmap

建立用户虚拟内存和内核缓冲区的映射, 这样 read 时不需要内核-用户空间的CPU拷贝, write 时直接CPU 从一个buf copy 到另一个buf

1. 切换状态: 用户调用mmap函数，从**用户态切换到内核态**
2. 映射: 建立文件物理地址和虚拟内存映射区域的映射，或者说是**内核缓冲区 (页高速缓存) 和虚拟内存**映射区域的映射
3. DMA拷贝: CPU 向磁盘 DMA 控制器发送读取指定位置和大小的指令，DMA 控制器将数据从磁盘拷贝到内核缓冲区
4. 切换状态: mmap系统调用结束返回，CPU 从**内核态切换到用户态**
5. 切换状态: 用户进程调用write函数，向内核发起调用，CPU 从**用户态切换到内核态**
6. CPU拷贝: CPU 将页高速缓存中的数据拷贝到套接字缓冲区
7. DMA拷贝: CPU 向磁盘 DMA 控制器发送 DMA 写指令，DMA 控制器从套接字缓冲区调用协议栈处理，最后把数据拷贝到网卡
8. 切换状态: write系统调用结束返回，CPU 从内核态切换到用户态。

> 全程需要4次切换, 2次DMA拷贝, 1次CPU拷贝

![](https://pica.zhimg.com/v2-98453b01b8f918c19f9fc30bc8850b1e_r.jpg)

### sendfile + DMA/SG-DMA: Better Way

- sendfile可以减少CPU拷贝, 从 kernel buffer直接发送到 I/O buffer
- 如果只支持DMA, CPU可以在两个设备对应的 kernel buffer之间建立 pipe, pipe也是不需要CPU拷贝的
- 如果支持SG-DMA, 那也不需要 pipe, 直接从 buf 到 buf

> 2次状态切换, 2次DMA拷贝, 0次CPU拷贝

![](https://pic1.zhimg.com/v2-a5e0cdad86ae1ddcdfaf22a1d0dd03c8_r.jpg)

![](https://pica.zhimg.com/v2-c7fcf91b053a433f4e66c46b08efd01e_1440w.jpg)
